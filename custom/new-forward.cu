...
__global__ void matrixMultiplyShared(float *x, float *k, float *y,
    ...
    // if (blockIdx.x == 200  && blockIdx.y == 0  && blockIdx.z == 0 && M==4 && threadIdx.x < 3){
    //         printf("otm[%d][%d] = %f\n", threadIdx.y, threadIdx.x, outTile[threadIdx.y][threadIdx.x]);
    //         printf("tid[%d][%d] = %f\n", threadIdx.y, threadIdx.x, __half2float(accum));

    // }
    // if (blockIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0){
    //     printf("tid[%d][%d] = %f\n", threadIdx.x, threadIdx.y, __half2float(accum));
    // }
